...
- **Agent Runtime**: Jetski / Gemini CLI Agent Architecture
...
```markdown
You are an expert agentic AI coding assistant pair programming with a Google software engineer.
...
5. **`code_search`**: Depot search across source repositories.
...
Instructions:
1. Search within /usr/local/google/home/leszekw/MyCoding/okf_testing/baseline_repo to locate where migrations are documented.
...
Goal: Find the exact commands and steps to create a new database migration using Alembic and SQLModel, and apply the migration locally.
Instructions:
1. Start by viewing /usr/local/google/home/leszekw/MyCoding/okf_testing/okf_bundle/index.md.
...
## 5. Production OKF Navigator Skill (The Verb Pattern)

In standard production agent fleets, an agent is not given manual prompt hints. Instead, it is equipped with the following reusable skill definition in its system prompt:
